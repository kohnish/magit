...
static msgpack_sbuffer msgpack_handler_buf_create(magit_res_T *res) {
    ...
    append_key_val_str(&packer, MAGIT_RES_KEY_LIST_Z, res->list_z);
    append_key_val_str(&packer, MAGIT_RES_KEY_BRANCH, res->branch);
    append_key_val_str(&packer, MAGIT_RES_KEY_HEAD_LOG_LINE, res->head_log_line);
    append_key_val_str(&packer, MAGIT_RES_KEY_UPSTREAM_BRANCH, res->upstream_branch);
    append_key_val_str(&packer, MAGIT_RES_KEY_UPSTREAM_SUBJ, res->upstream_subj);
    append_key_val_str(&packer, MAGIT_RES_KEY_TAG_DESC, res->tag_desc);
    append_key_val_str(&packer, MAGIT_RES_KEY_OPT_TAG_DESC_HEAD, res->opt_tag_desc_head);
    append_key_val_str(&packer, MAGIT_RES_KEY_VERSION, res->version);
    append_key_val_str(&packer, MAGIT_RES_KEY_WORKTREE_PORCELAIN, res->worktree_porcelain);
    append_key_val_str(&packer, MAGIT_RES_KEY_DOT_GIT_DIR, res->dot_git_dir);
    append_key_val_str(&packer, MAGIT_RES_KEY_CONFIG_STATUS_SHOW_UNTRACKED_FILES, res->config_status_show_untracked_files);
    append_key_val_str(&packer, MAGIT_RES_KEY_STATUS, res->status);
    append_key_val_str(&packer, MAGIT_RES_KEY_DIFF, res->diff);
    append_key_val_str(&packer, MAGIT_RES_KEY_STAGED, res->staged);
    ...
    append_key_val_str(&packer, MAGIT_RES_KEY_LOGS, res->logs);
    append_key_val_str(&packer, MAGIT_RES_KEY_REV_SHORT_HEAD, res->rev_short_head);
    append_key_val_str(&packer, MAGIT_RES_KEY_TAG_MASTER, res->tag_master);
    append_key_val_str(&packer, MAGIT_RES_KEY_TAG_ORIGIN_MASTER, res->tag_origin_master);
    append_key_val_str(&packer, MAGIT_RES_KEY_ORIGIN_HEAD, res->origin_head);
    append_key_val_str(&packer, MAGIT_RES_KEY_TAG_ORIGIN_HEAD, res->tag_origin_head);
    append_key_val_str(&packer, MAGIT_RES_KEY_UPSTREAM_BRANCH_MASTER, res->upstream_branch_master);
    return buf;
}
